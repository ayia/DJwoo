...
public class FragmentYPYDetailTracks extends DBFragment implements IXMusicConstants, IYPYMusicConstant {
    ...
    public void startLoadData() {
        if (mContext != null && mRecyclerView != null) {
            ...
            DBExecutorSupplier.getInstance().forBackgroundTasks().execute(new Runnable() {
                ...
                @Override
                public void run() {
                    final ArrayList<TrackModel> mListTracks = mContext.mTotalMng.getAllTrackWithAdmob(mContext, getListTracks(), mTypeUI);
    ...
    private ArrayList<TrackModel> getListTracks() {
        ...
        } else if (mTypeDetail == TYPE_DETAIL_RECOMMENDED) {
            ArrayList<TrackModel> mListTracks = mContext.mTotalMng.getListTopMusicObjects();
            return mListTracks;
    ...
    private void setUpInfo(final ArrayList<TrackModel> mListDatas) {
        ...
        if (mListDatas != null && mListDatas.size() > 0) {
            mTrackAdapter = new TrackAdapter(mContext, mListDatas, mContext.mTypefaceBold
